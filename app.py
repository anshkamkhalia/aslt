...
model = load_model(
    "best_model.keras", 
    custom_objects={"Translator": Translator, "Attention": Attention}
)
...
# Buffers for sequence and stable prediction
...
def predict_sign():
    try:
        data = request.json
        frames = data.get("frames")  # Expecting list of base64-encoded frames
        ...
        for b64_frame in frames:
            ...
            if results.multi_hand_landmarks:
                ...
                keypoints = np.array(
                    sum([[lm.x - base_x, lm.y - base_y, lm.z - base_z] 
